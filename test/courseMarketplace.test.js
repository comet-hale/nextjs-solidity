...
contract("CourseMarketplace", (accounts) => {
  ...
  describe("Purchase the new course", () => {
    ...
    it("can get the purchased course hash by index", async () => {
      ...
      assert.equal(
        courseHash,
        expectedHash,
        "Course hash is not matching the hash of purchased one"
      );
    });
  ...
  describe("Repurchase course", () => {
    ...
    it("should be able repurchase with the origin buyer", async () => {
      const beforeTxBuyerBalance = await getBalance(buyer);
      const beforeTxContractBalance = await getBalance(_contract.address);
      ...
      assert.equal(course.price, value, `The course is not equal to ${value}`);
  ...
  describe("Emergency withdraw", () => {
    ...
    it("should have contract balance of 0", async () => {
      ...
      assert.equal(contractBalance, 0, "Owner doesn't have 0 balance");
    });
  });
